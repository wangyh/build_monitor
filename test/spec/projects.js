describe("Projects", function(){
	...
	it("should be able to filter projects", function(){
		var projects_3 = new Projects(
			[
				...
				{lastbuildstatus: "Success", name:"Pipeline4 :: Stage4"},
				{lastbuildstatus: "Success", name:"Pipeline4 :: Stage5"}
			]);
		expect(projects_3.grepByFilter({include:[/Stage1/]}).projects.length).toEqual(1);
		expect(projects_3.grepByFilter({include:[/Stage1/]}).projects[0].name).toEqual("Pipeline1 :: Stage1");
		
		expect(projects_3.grepByFilter({exclude: [/Pipeline1/]}).projects.length).toEqual(4);
		expect(projects_3.grepByFilter({include: [/Pipeline4/], exclude: [/Stage4/]}).projects.length).toEqual(1);
	});
